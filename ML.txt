    ...
    "rfsrc" = {
      ...
      if (requireNamespace("randomForest", quietly = TRUE)) {
        ...
        if ("MeanDecreaseAccuracy" %in% colnames(imp)) {
          importance_scores <- imp[, "MeanDecreaseAccuracy"]
        } else if ("%IncMSE" %in% colnames(imp)) {
          importance_scores <- imp[, "%IncMSE"]
        } else {
          importance_scores <- imp[, ncol(imp)]
        }
        # 选择重要性高于中位数的变量
        threshold <- median(importance_scores, na.rm = TRUE)
        names(importance_scores)[importance_scores > threshold & !is.na(importance_scores)]
      } else {
        fit$subFeature # 回退到使用所有特征
      }
    },
    "gbm" = {
      gbm_imp <- summary.gbm(fit, plotit = F)
      rownames(gbm_imp)[gbm_imp$rel.inf > 0]
    },
